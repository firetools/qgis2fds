...
class FDSCase:
    ...
    def get_fds(self):
        ...
        version_str = f"""qgis2fds {plugin_version}"""
        
        try:
            import git
            systemdir = os.sep.join(__file__.split(os.sep)[:-2])
            repo = git.Repo(systemdir)
            sha = repo.head.object.hexsha
            githash = sha[:7]
            if repo.is_dirty():
                githash = githash + '-dirty'
            version_str = f"""{version_str} commit ({githash})"""
        except:
            pass
        
        # Prepare fds case
        return f"""\
! Generated by {version_str} on QGIS {qgis_version}
